...
void video_processing() {
    std::string command = "python3 ./cam/video_processing.py";  // 실행할 파이썬 스크립트 명령어 (유닉스에서는 python3 사용)
    ...
    while (fgets(buffer, sizeof(buffer), pipe) != NULL) {  // 파이프에서 데이터 읽기
        ...
        try {
            json message = json::parse(json_str);  // 문자열을 JSON 객체로 파싱
            ...
            std::cerr << "JSON parsing error: " << e.what() << " for data: " << json_str << std::endl;  // 오류 메시지 출력
        }
    }
...
void handle_client(SOCKET client_socket) {
    ...
    while (true) {
        ...
        try {
            ...
            std::cerr << "JSON parsing error: " << e.what() << " for data: " << received_data << std::endl;  // 오류 메시지 출력
        }
...
void run_stt_tts() {
    std::string command = "python3 ./bllossom/bllossom_stt_tts.py";  // 실행할 파이썬 스크립트 명령어 (유닉스에서는 python3 사용)
    std::cout << "Executing command: " << command << std::endl;  // 명령어 출력
...
void process_messages(SOCKET client_socket) {
    std::thread video_thread(video_processing);  // 비디오 처리 스레드 생성

    while (true) {
        std::unique_lock<std::mutex> lock(mtx);  // 뮤텍스 잠금
        cv.wait(lock, []{ return !message_queue.empty(); });  // 조건 변수를 이용해 메시지 큐가 비어있지 않을 때까지 대기

        while (!message_queue.empty()) {  // 메시지 큐가 비어있지 않은 동안
            std::string message = message_queue.front();  // 큐의 첫 번째 메시지 가져오기
            message_queue.pop();  // 큐에서 메시지 제거
            lock.unlock();  // 뮤텍스 잠금 해제

            try {
                json msg = json::parse(message);  // 메시지를 JSON으로 파싱
                std::string message_type = msg["message_type"];  // 메시지 유형 추출
                std::string value = msg["value"];  // 메시지 값 추출

                std::cout << "Processed message: " << message << std::endl;  // 처리된 메시지 출력

                // 특정 메시지 유형 처리
                if (message_type == "client_classification" && value == "Wheelchair") {  // 메시지 유형이 "client_classification"이고 값이 "Wheelchair"인 경우
                    json response = {
                ...
                std::cerr << "JSON parsing error: " << e.what() << std::endl;  // 오류 메시지 출력
            }
    ...
    if (video_thread.joinable()) {  // 비디오 처리 스레드가 joinable 상태인지 확인
        video_thread.join();  // 비디오 처리 스레드 종료 대기
    }
}
...
int main() {
    ...
    while (true) {
        ...
        SOCKET client_socket = accept(server_fd, (struct sockaddr*)&address, (socklen_t*)&address.sin_addr.s_addr);  // 클라이언트 연결 수락
