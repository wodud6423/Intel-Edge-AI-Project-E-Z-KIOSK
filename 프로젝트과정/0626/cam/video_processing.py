...
model = YOLO('./model/best.pt')  # YOLO 모델 경로 설정
...
# 탐지 주기 설정 (프레임당 대기 시간)
detection_interval = 1.0  # 초단위 설정 (예: 1초)
...
while True:
    ...
    if detections:
        ...
        if class_name == 'wheelchair':
            json_data = {
                "message_type": "client_classification",
                "value": "Wheelchair"
            }
            print(json.dumps(json_data))  # JSON 데이터를 표준 출력으로 전송
            logger.info(f"Sent: {json_data}")  # 로그 기록
